...
Zotero.Translate.prototype._itemDone = function(item, attachedTo) {
	...
	if(type == "note") {	// handle notes differently
		...
		var itemTypeID = Zotero.ItemTypes.getID(type);
		var itemTypeFields = Zotero.ItemFields.getItemTypeFields(itemTypeID);
		// loop through ordinary multilingual fields
		for (var i = 0, ilen = item.multi.length; i < ilen; i += 1) {
			var fieldName = item.multi[i].fieldName;
			// No orphan multilingual fields.
			if (!item[fieldName]) {
				continue;
			}
			var myFieldID = Zotero.ItemFields.getID(fieldName);
			if (itemTypeFields.indexOf(myFieldID) === -1) {
				continue;
			}
			// Normalize lang
			// Patch code by Florian Ziche.
...
Zotero.Translate.RDF.prototype.addStatement = function(about, relation, value, literal, lang) {
	if(literal) {
...
Zotero.Translate.RDF.prototype.getTargets = function(resource, property, preserveObject) {
	var statements = this._dataStore.statementsMatching(this._getResource(resource), this._getResource(property));
	if(!statements.length) return false;
	if (preserveObject) {
		var ret = [s.object for each(s in statements)];
	} else {
		var ret = [(s.object.termType == "literal" ? s.object.toString() : s.object) for each(s in statements)];
	}
	return ret;
};
