...
Zotero.Item.prototype.__defineGetter__('firstCreatorSort', function () { return this.getField('firstCreatorSort'); });
 ...
 */
Zotero.Item.prototype.getField = function(field, unformatted, includeBaseMapped, language) {
	...
	// XXXZ change for multi
	if (Zotero.multiFieldIds[fieldID]  && !unformatted) {
		var value = this.getMultiField(fieldID, language);
	} else {
...
// XXXZ
Zotero.Item.prototype.getMultiField = function (fieldID, langs, honorEmpty) {
	var val, lang;
	...
	if ("object" === typeof langs) {
		for (var i = 0, ilen = langs.length; i < ilen; i += 1) {
			if (this._multiField[fieldID] && this._multiField[fieldID][langs[i]]) {
				lang = langs[i];
				break;
			}
		}
	...
	// Special language option ALL forces creation of a composite
	// field containing all language variants.
	if (lang && lang !== 'ALL' && this._multiField[fieldID] && this._multiField[fieldID][lang]) {
		val = this._multiField[fieldID][lang];
	} else if (!honorEmpty) {
		if (this._itemData[fieldID] && this._itemData[fieldID].slice(0, 1) === '#') {
			val = this._multiBase[fieldID];
			if (lang === 'ALL') {
				for (var tag in this._multiField[fieldID]) {
					val += ' :' + tag + ': ' + this._multiField[fieldID][tag];
				}
			}
		} else {
			val = this._itemData[fieldID];
		}
	}
	return val ? val : '';
};
...
Zotero.Item.prototype.loadPrimaryData = function(allowFail) {
	...
	var firstCreatorTable = Zotero.Items.setFirstCreatorTable();
	...
	for each(var field in Zotero.Items.primaryFields) {
		...
		if (field == 'itemID' || this['_' + field] === null) {
			...
			switch (field) {
				...
				case 'firstCreator':
					colSQL = Zotero.Items.getFirstCreatorSQL(firstCreatorTable);
					break;

				case 'firstCreatorSort':
					colSQL = Zotero.Items.getFirstCreatorSortSQL(firstCreatorTable);
					break;
	...
	Zotero.Items.unsetFirstCreatorTable(firstCreatorTable);
...
Zotero.Item.prototype.setType = function(itemTypeID, loadIn) {
	...
	if (this._itemTypeID) {
		...
		// XXXZ Just scrub multilinguals to force item-internal cache reset?
		// Actually, this seems to happen on it's own.
		//this._multiBase = {};
		//this._multiField = {};
	...
	// XXXZ Apparently unnecessary, holding in reserve for the present.
	//for (var key in this._itemData) {
	//	if (Zotero.multiFieldIds[key]) {
	//		this._loadMulti(key);
	//	}
	//}
...
// MMM I think this is where we splice in multilingual.
//
// Multi values store _before_ the headline value, with a
// marker prefix on the field. Such fields are a special
// object, with a setter/getter that will handle string
// assignments preceded by a language marker appropriately,
// splicing them in to the underlying field string. When
// multilingual content is present on the field, an attempt
// to set the headline entry to nil or an empty string
// should throw an error.  UI should not permit this
// to happen.
//
// The getter for the fieldname just delivers the
// headline content.
//
// A companion field with a suffix -multi on the fieldname
// is an object keyed to getters that fetch out their respective content
// from the string.  The raw object is screened from
// view, with an accessor that instantiates the object on
// new keys as required. ... not sure about this, maybe
// it can be simplified ... but with {} seems to offer
// a promising means of doing something like this without
// copying functions all over the place.
//
// A second companion field the a -raw
// suffix on the fieldname fetches out the raw string
// (for use in sync, for example).
//
// This jazz can all be installed by a factory function,
// which should be built and tested in a framework before
// bringing it here.
...
Zotero.Item.prototype.setMultiField = function (field, val, lang) {
	...
	Zotero.debug("Zotero.Item.prototype.setMultiField: " + field, 
			+ ", " + val + ", " + lang, 3);

	// Ban the hash as initial character in the text segment of a
	// multilingual field, as it's a marker reserved for internal use.
	...
	// Skip if field not used in this itemType
	var itemTypeFields = Zotero.ItemFields.getItemTypeFields(this.itemTypeID);
	if (itemTypeFields.indexOf(fieldID) === -1) {
		throw "setMultiField: field does not exist in itemType";
	}
	...
	if (!lang) {
		...
		//Normalize lang
		if(Zotero.ZlsValidator.validate(lang)) {
			lang = [Zotero.ZlsValidator.tagdata[i].subtag for (i in Zotero.ZlsValidator.tagdata)].join("-");
		}
		else {
			Zotero.debug("Language validation failed: " + lang, 2);
		}
		...
		if (!val && this._multiField[fieldID][lang]) {
			...
			Zotero.debug("Set _multiField[" + fieldID + "][" + lang +"]");
		}
	}
	//
	if (!this._multiField[fieldID] || !this._multiField[fieldID].__count__) {
...
Zotero.Item.prototype.getDisplayTitle = function (includeAuthorAndDate, language) {
	...
	if (!title && (itemTypeID == 8 || itemTypeID == 10)) { // 'letter' and 'interview' itemTypeIDs
		var creators = this.getCreators(language);
		var authors = [];
		var participants = [];
		if (creators) {
		...
		var reporter = this.getField('reporter', false, false, language);
		if (reporter) {
...
Zotero.Item.prototype.moreCreatorsAfter = function(index) {
	var i, ilen;
	if (this.id && !this._creatorsLoaded) {
		this._loadCreators();
	}
	for (i = index + 1, ilen = this._creators.length; i < ilen; i += 1) {
		if (!this._creators[i].ref.servantLang) {
			return true;
		}
	}
	return false;
 ...
 * Note: Creator data array is returned by reference
 */
Zotero.Item.prototype.getCreators = function(language) {
	var i, ilen;
	if (this.id && !this._creatorsLoaded) {
		this._loadCreators();
	}
	var creator_list = this._creators;
	if (language) {
		creator_list = [];
		for (i = 0, ilen = this._creators; i < ilen; i += 1) {
			if (!this._creators[i].servantLang) {
				creator_list.push(this._creators[i]);
			} else if (this._creators[i].servantLang === language) {
				creator_list.pop();
				creator_list.push(this._creators[i]);
			}
		}
...
Zotero.Item.prototype.save = function() {
	...
	// XXXZ Reconcile masterIndex on all servant entries
	var masterIndex = 0;
	for (var i = 0, ilen = creators.length; i < ilen; i += 1) {
		if (!creators[i].ref.servantLang) {
			masterIndex = i;
		} else {
			if (creators[i].ref.masterIndex != masterIndex) {
				creators[i].ref.masterIndex = masterIndex;
				creators[i].ref.save();
			}
		}
	}
...
Zotero.Item.prototype.toArray = function (mode) {
	...
	for (var i in this._itemData) {
		var value = this._itemData[i] ? this._itemData[i] + '': '';
		var fieldName = Zotero.ItemFields.getName(i);
		if (value && value[0] === '#') {
			[value, multi] = Zotero.Multi.parseSerializedMultiField(value);
			if (value) {
				for (var langTag in multi) {
					if (!arr.multi[fieldName]) {
						arr.multi[fieldName] = {};
					}
					arr.multi[fieldName][langTag] = multi[langTag];
				}
			}
		}
		arr[fieldName] = value;
	...
	if (!this.isNote() && !this.isAttachment()) {
		...
		for (var i in creators) {
			...
			creator.servantLang = creators[i].ref.servantLang;
			creator.multi = {};
			if (creator.servantLang) {
				arr.creators[arr.creators.length - 1].multi[creator.servantLang] = creator;
			} else {
